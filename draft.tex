...
interleaved with some non-looping code. These loops are executed
...
loop, and then verifying these reducers separately.
\footnote{Nested
loops can be removed by adding additional branch statements, since both the inner and outer loop are over the same input list.
In fact, such program construct never occured in examples we have ever seen.} The top-level
...
will work with random initial values of the shared variable, which over-approximates the original behavior. Usually, the second (or further) top-level loop are commutative even with arbitary initial shared variable values and hence J-ReCoVer can be used to prove it.
...
\end{wrapfigure}  
For the $115$ solved cases, $72\%$ of the execution time are spent in compiling JAVA source code to bytecode, which is the input of the Soot tool. Another $27\%$ of the execution time are spent in the Propossesor, in which Soot is used to transform JAVA bytecode to Jimple and perform the program transformations. The time spent in Solver is quite limited, since the real-world integer reducer programs are usually not that big. 
...
There is no other tool that could handle the reducers as they are. Perhaps some other tools could be applied on the transformed programs, but the transformations would still be needed, and our SMT-based back-end verifier turned out to work efficiently, and so we did not feel a need to replace it by another verification tool. Of course, in the future, this can be done if need be.
