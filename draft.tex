...
the presentation, we assume that a reducer does never produce output in the loop
body $s_2$. In the J-ReCoVer tool, an algorithm to deal with output inside the
loop is implemented.

% !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
%
% Just to remember: the way the output from the main loop is handled currently
% is wrong: One should either check that there is no flow between the input and
% output in the loop, or to add a check that the projection of the F function on
% any value output in the main loop stays the same for any input value.
%
% !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
iteration. When multiple reads from the input occur in a single branch of the
loop, we can use additional variables and break the branch into several
