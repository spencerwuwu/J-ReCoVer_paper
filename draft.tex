...
\title{\hspace{-1mm}{J-ReCoVer}:~Java~Reducer~Commutativity~Verifier\thanks{The
work was supported by the Minister of Science and Technology of Taiwan project 106-2221-E-001-009-MY3 and Czech Science Foundation project
17-12465S.}}

\author{
Yu-Fang Chen\inst{1,2}
\and
Chang-Yi Chiang\inst{2}
\and
Luk\'{a}\v{s} Hol\'{\i}k\inst{3}
\and
Wei-Tsung Kao\inst{1}
\and
Hsin-Hung Lin\inst{1}
\and
Tom\'{a}\v{s} Vojnar\inst{3}
\and
Yean-Fu Wen\inst{2}
\and
Wei-Cheng Wu\inst{1}
...
\begin{abstract} The MapReduce framework for data-parallel
computation was first proposed by Google~\cite{dean04} and later implemented in
the Apache Hadoop project. Under the MapReduce framework, a reducer computes
output values from a sequence of input values transmitted over the network. Due
to non-determinism in data transmission, the order in which input values arrive
at the reducer is not fixed. In relation to this, the \emph{commutativity
problem} of reducers asks if the output of a reducer is independent of the order
of its inputs. Indeed, there are several advantages for a reducer to be
commutative, e.g., the verification problem of a MapReduce program can be
reduced to the problem of verifying a sequential program. We present the tool
J-ReCoVer (Java Reducer Commutativity Verifier) that implements effective
heuristics for reducer commutativity analysis. J-ReCoVer is the first tool that
is specialized in checking reducer commutativity. Our experimental results over
118 benchmark examples collected from open repositories are very positive;
J-ReCoVer correctly handles over 97~\% of them. \end{abstract}
...
Various forms of sequentialisation are also used in works dealing with the
concept of \emph{robustness} of event-driven asynchronous
programs~\cite{ahmed2017:robustness} or works dealing with programs running
under some \emph{relaxed memory
models}~\cite{ahmed2013:robustness,AbdullaACLR13,AbdullaACLR12}. However, their
computation models are again quite different from that of reducers, and their
results cannot be directly applied. Besides verification, another interesting
research direction, using commutativity analysis as a component, is
\emph{synthesis} of MapReduce programs~\cite{SmithA16}.
...
%%
%The language allows us to give examples of reducers and
%to discuss our approach for checking their commutativity. 
%
...
loop body $s_2$. J-ReCoVer implements an algorithm to deal with an output
inside the loop (as briefly mentioned at the end of
...
framework. The J-ReCoVer tool has three main components,
\emph{Preprocessor}, \emph{Prover}, and \emph{BugFinder}. As the name
suggests, Preprocessor reads as input a reducer program and performs the
...
and uses the tool Soot\footnote{\url{https://github.com/Sable/soot}} to further convert it to the so-called ``Jimple'' format,
...
The \emph{BugFinder component} generates random pairs of lists, 
with the list of each pair being permutations of each other. 
A concrete counterexample is reported if
the reducer outputs different results for the two lists of a generated pair. 
Our procedure for generating random pairs is quite naive. We
use five different input list of lengths 5, 7, 9, 11, and 13. For each length,
we generate 100 lists and pick uniformly at random one of its permutations.
Although the approach is simple; in practice, it finds counterexamples in all of
our non-commutative benchmarks in few seconds. 

The \emph{Prover component} reduces the commutativity problem to an SMT
problem. From a high-level point of view, we are checking equivalence
between a reducer program and its variant that has two consecutive inputs
swapped. We show that this equivalence check can be reduced to a first-order formula and give it
...
iteration. When multiple reads from the input occur in a single execution path from the begin to the end of the loop body, we can use additional variables and branch statements to break the path into several auxiliary ones, each reading just once. 

The command $s_2$ can be viewed as a function $F$ that reads the values of all variables and the current input before executing $s_2$ and outputs the values of all variables after $s_2$. The loop body $s_2$ contains no nested loop structure. Hence, a bounded summary in quantifier-free linear integer arithmetic is sufficient~for~describing~$F$.

Formally, the function $F(n,x_1,x_2,\ldots,x_k): \Z^{k+1}
...
In our initial experiments, we realised that Formula~\ref{eq:commu} is too strong, 
too often violated by reducers that are commutative.
%
To illustrate
...
After the loop, the value of $s$ is output. In this case, the function $F$ 
...
together with a support for more data types, is among our future directions.
